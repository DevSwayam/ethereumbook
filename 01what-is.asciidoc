...
From a computer science perspective, Ethereum is a <bold>deterministic (GIven particular input to ethereum computer will do same action and will produce same output everytime)</bold> but practically unbounded state machine, consisting of a globally accessible singleton state and a virtual machine that applies changes to that state.
